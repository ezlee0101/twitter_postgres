...
for file in $files; do
    python3 load_tweets.py --db postgresql://postgres:pass@localhost:4116/postgres --inputs "$file" --print_every 10000
done


echo 'load denormalized'
for file in $files; do
    unzip -p "$file" | sed 's/\\u0000//g' | psql postgres://postgres:pass@localhost:4115/postgres -c "COPY tweets_jsonb (data) FROM STDIN csv quote e'\x01' delimiter e'\x02';"   
